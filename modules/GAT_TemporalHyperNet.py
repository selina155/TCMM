...
from src.modules.MultiEmbedding import Embedding
...
class TemporalHyperNet(pl.LightningModule):
    ...
    def __init__(self,
        ...
        self.th_embeddings=Embedding(num_nodes=self.num_nodes,lag=self.lag,embedding_dim=self.embedding_dim)
    ...
    def forward(self, X: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, ...]:
        ...
        assert (A.shape[0] == batch and A.shape[1] == lag and A.shape[2] == num_nodes and A.shape[3] == num_nodes)

        if embeddings is None:
            E = self.th_embeddings.get_embeddings().expand(
                X_in.shape[0], -1, -1, -1
            )
        else:
            E = embeddings
        ...
        X_sum = torch.einsum("blij,bljd->bid", A_temp, Wh)
